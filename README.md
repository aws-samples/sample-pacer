...
`aws-efa-k8s-device-plugin` installed, layer
[values-efa.yaml](deploy/helm/pacer/values-efa.yaml) (`efa.enabled: true`) onto your
install. The daemon probes for EFA at startup and speaks gRPC-only where it is absent, and
...
hot-swap the binary in running pods), see [scripts/dev/README.md](scripts/dev/README.md):
...
| [scripts/dev/](scripts/dev) | Local dev loop (cross-compile + hot-swap binary into running pods) |
