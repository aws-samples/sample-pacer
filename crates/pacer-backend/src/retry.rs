...
impl RetryPolicy {
    ...
    /// that ceiling. `salt` decorrelates *concurrent* reads: the pipeline has
    /// `fill_parallelism` chunks in flight, and a backend-wide fault fails them
    /// together, so an unjittered policy would retry all of them in lockstep
    /// and re-create the burst that caused it. Passing the chunk index as the
    /// salt spreads them deterministically — no PRNG, and the spread is
    ...
    pub fn backoff(&self, attempt: u32, salt: u64) -> Duration {
        ...
        let fraction = mix(salt, attempt) % JITTER_SCALE;
...
pub struct ChunkRead<'a> {
    ...
    pub range: Range<u64>,
    /// Jitter decorrelation salt; pass the chunk index (see
    /// [`RetryPolicy::backoff`]).
    pub salt: u64,
}
...
pub async fn read_range(
    ...
    retrying(policy, read.salt, || read_once(client, read)).await
}
    ...
    Fut: Future<Output = Result<Bytes, AttemptError>>,
{
    ...
    for attempt in 1..=budget {
        ...
        let delay = policy.backoff(attempt, salt);
        warn!(
            attempt,
            budget,
...
/// One splitmix64 finalizer over `salt` and `attempt` — a cheap avalanche that
/// gives every (chunk, attempt) pair its own jitter fraction with no `rand`
...
fn mix(salt: u64, attempt: u32) -> u64 {
...
mod tests {
    ...
    fn concurrent_chunks_do_not_retry_in_lockstep() {
        // The point of the salt: `fill_parallelism` chunks failing together
