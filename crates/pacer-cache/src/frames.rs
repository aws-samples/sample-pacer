...
/// Whether this node installed a frame source at all — i.e. whether it HAS a slab.
///
/// The distinction this exists for, and it is the whole reason a caller cannot just call
/// [`claim`]: `claim` returns `None` for two unrelated reasons. **No slab** is a
/// configuration state, fixed for the process and knowable at startup. **No free frame** is
/// transient pressure, and the heap fallback under it is correct — a read must still be
/// served. Only the first is a misconfiguration, so only the first can be refused, and
/// nothing else in this module can tell them apart.
///
/// `store`'s reads are the caller that cares: `O_DIRECT` needs a page-aligned buffer, a
/// heap `Vec` is not aligned, so with no slab EVERY store read silently takes the buffered
/// descriptor — 26.9-34.9 ms of service time per 16 MiB against 17.3 direct, and it
/// pollutes the page cache the tier exists to bypass. The daemon checks this once after
/// startup rather than discovering it per read.
#[must_use]
pub fn has_frame_source() -> bool {
    FRAME_SOURCE.get().is_some()
}
