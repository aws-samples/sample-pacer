...
/// Build the chunk cache with ADR-0033's slot store as foyer's **disk-cache engine**
/// (prototype — see [`store_engine`]).
///
/// The difference from [`build_chunk_cache_with_metrics`] is one line: the storage phase is
/// given a [`store_engine::ChunkEngineConfig`] instead of a `BlockEngineConfig`. Everything
/// above the disk tier is unchanged — the RAM tier, its LRU and weighter, `get_or_fetch`'s
/// single-flight coalescing, promotion — which is the whole reason this seat is worth using
/// rather than replacing the cache outright as `diskTier=store` does.
///
/// # Errors
///
/// Fails when the cache directory cannot be created, the device cannot be opened/sized,
/// `uring` is requested on a non-Linux target, or the slot store cannot be opened.
/// Returns the cache **and** the store behind it: the daemon publishes the store's
/// counters, and a store sealed inside the engine could not be reached.
pub async fn build_chunk_cache_with_engine(
    cfg: &CacheConfig,
    store: store::StoreConfig,
) -> anyhow::Result<(ChunkCache, store::ChunkStore)> {
    // Refused rather than left to deadlock. With the composite engine `disk_capacity` sizes
    // the HEADER half — foyer's device — and a device with room for zero blocks does not
    // fail to build: the block engine's reclaimer waits for a clean block that can never
    // exist, so the symptom is a hang with no log line, not an error. Cost one debugging
    // round on a test fixture that paired a 64 MiB block with a 288 KiB device.
    // (`build_chunk_cache` has the same latent trap; it is left alone here because changing
    // the shipped path's failure mode is not this function's business.)
    anyhow::ensure!(
        cfg.disk_capacity >= cfg.block_size,
        "disk_capacity ({}) must be at least one block_size ({}): foyer's device would hold \
         zero blocks, and the header half of the composite engine would stall rather than error",
        cfg.disk_capacity,
        cfg.block_size,
    );
    let device = open_device(cfg).await?;
    // Opened here, not inside the engine's `build`, so this function can hand it back.
    // `open` scans slot headers to rebuild the index — 48 ms for a 200 GiB tier on
    // hardware — so a restart keeps the tier exactly as the standalone path does.
    let store = store::ChunkStore::open(store).await?;
    let storage = HybridCacheBuilder::new()
        ...
        .storage()
        .with_io_engine_config(io_engine_config(cfg)?)
        // Coerced explicitly: `with_engine_config` takes `impl Into<Box<dyn EngineConfig>>`
        // and foyer has no blanket `From<T>` for it, so an unboxed config does not convert.
        .with_engine_config(Box::new(store_engine::ChunkEngineConfig::new(
            Arc::clone(&device),
            store.clone(),
            // The header half. Reusing `block_engine_config` rather than spelling a
            // block engine out here is what keeps every `StorageTuning` knob applying
            // to both tiers — a `flushers` value that reached only the non-composite
            // path would be a silent difference between two deployments of one chart.
            Box::new(block_engine_config::<String, CacheValue>(cfg, device)),
        ))
            as Box<dyn foyer::EngineConfig<String, CacheValue, HybridCacheProperties>>);
    Ok((storage.build().await?, store))
}
