...
//! syscall. The reads themselves are `pread` on `&File` from `spawn_blocking`, so any
//! number proceed at once; that is the whole point, since the device needs ~32
//! concurrent 16 MiB reads to reach 43.565 GiB/s and one thread already reaches 11.696.
//!
//! # On-disk layout
//!
//! ```text
...
impl ChunkStore {
    ...
    pub async fn get(&self, key: &str) -> anyhow::Result<Option<CachedChunk>> {
        ...
        let read = tokio::task::spawn_blocking(move || inner.read_slot(&key, slot)).await?;
        match read {
            Ok(Some(chunk)) => {
...
impl Inner {
    ...
    fn read_slot(&self, key: &str, slot: u32) -> anyhow::Result<Option<CachedChunk>> {
        ...
        let header = match header {
            Ok(header) => header,
            Err(SlotState::Free) => return Ok(None),
            Err(SlotState::Corrupt | SlotState::KeyNotUtf8) => {
                StoreStats::inc(&self.stats.corrupt_slots);
                return Ok(None);
            }
        };
        // THE check that replaces foyer's XxHash64 for the failure this code can cause. It
        // still reads the key off DISK rather than from the index, which would only be
        ...
        // which reserves `chunk_size` whatever the body is — into a frame claimed at that
        // size, then publish only `body_len`. Slicing a sealed frame keeps the same
        // allocation, so this costs a refcount and not a copy.
        let read_len = align_up(len).min(self.cfg.chunk_size);
        let body = match frames::claim(read_len) {
...
mod tests {
    ...
    /// A store of exactly `slots` slots in a fresh temp dir.
