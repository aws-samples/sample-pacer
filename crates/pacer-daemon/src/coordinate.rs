...
async fn upload_here(
    ...
    // `disable_payload_signing`: see `peer.rs`'s `upload_scattered_part`, the same call
    // for the same reason — without it the SDK SHA-256s the whole window inline on this
    // worker thread before the first byte moves, to sign a body the CRC32 above already
    // integrity-checks over TLS.
    let out = shared
        .backend
        .upload_part()
        .bucket(bucket)
