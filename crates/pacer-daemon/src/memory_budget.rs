...
///
/// **The boundary, measured over every overlay in the repo (2026-09-08):** of 34
/// renders — `values.yaml`, all seven `deploy/helm/pacer/values-*.yaml`, all 24
/// `bench/ladder/values-*.yaml` layered on `values-ladder.yaml`, and the three
/// `pacer-dev` layerings — **every one passes**, most with 3-10× margin. Exactly one
/// render does not: `values-dev-gpu.yaml` **on its own**, where a 4 GiB tier, an 8 GiB
/// arena pair and an 8 GiB slab total 20 GiB against a 20 GiB rendered limit, i.e. base
/// == tier. That file is never deployed that way — its own header says it is layered by
/// `scripts/dev/pacer-dev` on top of `values-gpu-ohio.yaml`, whose 8 GiB base gives a
/// 24 GiB limit and 2 GiB of slack — and it is the very file
/// `pacer.validateMemoryBudget` names as why equality still renders. Rendering it alone
/// to reason about the arithmetic is fine; deploying it alone would be the incident.
