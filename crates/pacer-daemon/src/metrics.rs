...
/// the `peer_rdma` path, where the holder's NIC writes the window instead.
...
pub struct DeliveryMetrics {
    ...
    /// [`Self::chunk_seconds`] split into the two stages a chunk actually spends
    /// it in, by `stage` — **the number the C4 read-path ladder could not get**.
    ///
    /// On a local hit `deliver_window` is exactly `cache_read` then `copy`, so
    /// these two sum to `chunk_seconds` and each is separately attributable.
    ...
    ///   below the chunk count is that path working rather than a gap.
...
fn register_rdma_gauges(registry: &Registry) -> anyhow::Result<RdmaMetrics> {
    Ok(RdmaMetrics {
        ...
        rail: gauge_vec(
            registry,
            "pacer_rdma_rail",
            "Per-rail RDMA state (planning/19 D5.1): metric=writes_in_flight|writes_total|write_bytes_total|ranges_in_use|staging_in_use|staging_ranges|ah_evictions, labelled by rail and its NUMA node",
            &["rail", "numa", "metric"],
        )?,
...
fn register_non_counters(
    registry: &Registry,
) -> anyhow::Result<(IntCounterVec, IntGauge, Gauge, Gauge, HistogramVec)> {
...
impl Metrics {
    ...
    pub fn new() -> anyhow::Result<Self> {
        ...
        Ok(Self {
            ...
            dir_rpc_seconds,
            cache_hits: c("pacer_cache_hits_total", "GETs served from cache")?,
            cache_misses: c("pacer_cache_misses_total", "Cacheable GETs that missed")?,
            cache_bypass: c(
