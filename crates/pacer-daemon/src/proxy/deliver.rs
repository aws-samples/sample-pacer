...
impl PacerProxy {
    ...
    pub(super) async fn deliver(
        ...
        // CRC32 is position-dependent, so windows fold in ASCENDING OFFSET. The fan-out is
        // `buffer_unordered`, which yields in COMPLETION order, so this sort is what makes the
        // digest well-defined at all — it is load-bearing, not defensive.
    ...
    async fn run_delivery(
        ...
        let mut resolving = futures::stream::iter(windows)
            ...
            // `buffer_unordered`, NOT `buffered`. Both keep N futures in flight, but
            // `buffered` yields in INPUT order, so a slot is freed only when the window at the
            // head finishes: one slow window (a store read that queued, a peer hop, a
            // backend miss) stalls admission for every window behind it while the others sit
            // completed in the buffer. That is head-of-line blocking on a fan-out whose whole
            // premise is that windows are disjoint destinations with no ordering constraint —
            // and the digest already sorts by `dst_at` below precisely so order is not relied
            // upon, which is what makes the unordered form a straight swap rather than a
            // trade.
            .buffer_unordered(self.delivery.parallelism.max(1));
