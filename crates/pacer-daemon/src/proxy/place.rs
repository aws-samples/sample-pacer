...
impl FillCtx {
    ...
    async fn place_window(
        ...
        match client_memory {
            ...
            ClientMemory::Token {
                ...
                // `checksum` is passed on rather than dropped with a `..`. It was dropped until
                // 2026-09-12, and `TokenWindow` does not carry it, so `write_window` had no way
                // to ask and digested unconditionally — see `digest_sent`.
                self.write_window(token, *checksum, window, body.slice(from..to), source)
                    .await
            }
        }
    }
    ...
    /// WRITE this window into a client-registered target, and digest the bytes we sent **if
    /// `checksum` asked for it**.
    ///
    ...
    /// probe once already.
    ///
    /// `checksum` is `ClientMemory::Token`'s own flag, threaded in because `TokenWindow` does
    /// not carry it: without it this function could not tell whether anyone wanted the digest,
    /// and until 2026-09-12 it therefore computed one always. See [`Self::digest_sent`].
    ///
    /// Both halves of the placement are timed — `rdma_write` and `digest` — so a token
    /// delivery's stages sum to `chunk_seconds` the way a mapped one's always have.
    ///
    /// # Errors
    ///
    ...
    async fn write_window(
        ...
        // Timed from HERE, and the guard covers the `?` and the decline below: the WRITE is the
        // token path's whole counterpart to `copy_window`, and until 2026-09-12 neither it nor
        // the digest below observed anything at all. That is what made 84–96 % of a pre-layout
        // 70B load a residual — `chunk_seconds` minus `cache_read`, attributable to "the
        // placement" only by subtraction (`vllm-prelayout-where-the-load-goes.md`).
        let write_timer = crate::metrics::ScopedStage::new(
            &self.metrics.delivery.stage_seconds,
            crate::metrics::DELIVERY_STAGE_RDMA_WRITE,
        );
        let outcome = efa
        ...
        // Dropped explicitly, so `rdma_write` ends where the WRITE does rather than absorbing
        // the digest below — which is a stage in its own right and the whole point of splitting
        // them.
        drop(write_timer);
        let len = src.len() as u64;
        let digest = self.digest_sent(checksum, src).await?;
        self.metrics
            .delivery
            .chunks
            .with_label_values(&[source])
    ...
    /// The source-side CRC32 over what this node just wrote — **only when `asked`**.
    ///
    /// `asked` is the token's own checksum flag, which is `spec.checksum` (`target.rs` builds
    /// the token from it), so it cannot disagree with what `run_delivery` will do with the
    /// result.
    ///
    /// # Why it is conditional
    ///
    /// It was not, until 2026-09-12, and that cost a full CRC32 pass over **every delivered
    /// byte** on every arm that asked for no checksum — 131.4 GiB on a pre-layout 70B load,
    /// on the same blocking pool the chunk store's `pread`s use, and discarded by
    /// `run_delivery`'s `(false, _)` arm. The remote half has always gated on the same flag
    /// (`peer.rs`), for the reason `token.rs` states on the field itself: verification is
    /// O(bytes) on the writer's CPU, so `checksum=none` has to reach whoever would pay it.
    /// This half is the writer too.
    ///
    /// # Errors
    ///
    /// The blocking task panicking. Cannot fail otherwise — a CRC32 has no error case.
    #[cfg(feature = "efa")]
    async fn digest_sent(&self, asked: bool, src: Bytes) -> S3Result<Option<DeliveryDigest>> {
        if !asked {
            return Ok(None);
        }
        let _timer = crate::metrics::ScopedStage::new(
            &self.metrics.delivery.stage_seconds,
            crate::metrics::DELIVERY_STAGE_DIGEST,
        );
