...
pub(super) enum ClientMemory {
    ...
    Token {
        ...
        /// it.
        ///
        /// **It has to reach the LOCAL writer too**, which this comment used to wave away
        /// with "every other arm digests where the flag is already in scope". It was in scope
        /// in `place_window` and dropped there with a `..`, so `write_window` had nothing to
        /// consult and digested unconditionally — a CRC32 over every delivered byte on every
        /// arm that asked for none. It is now passed on explicitly (2026-09-12).
        checksum: bool,
    },
}
