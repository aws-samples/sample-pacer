...
pub struct ServedNode {
    ...
    /// TCP connections this node's peer server has accepted.
    ///
    /// Counted at the listener because the peer plane's connection count is now a
    /// configured quantity (`cluster.peerConnections`), and no RPC-level counter can see
    /// it: a pool of N that silently dialled once would serve every call correctly and
    /// measure the one-flow arm under the wide arm's name.
