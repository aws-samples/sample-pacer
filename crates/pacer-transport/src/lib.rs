...
/// so the connection row is a per-peer total shared by every in-flight window, not a
/// per-window allowance. `results/laguna-store-remotewrite.md` measured the save's
