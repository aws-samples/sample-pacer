...
earlier overlay's pool label would otherwise render an API-invalid null value.
...
{{/*
Which implementation owns the disk tier: `store` or `foyer` (ADR-0033, ADR-0038).

**This is where the store is turned ON by default**, and it is gated on THE SLAB EXISTING for
the same reason `pacer.cacheSlabBytes` is gated on hugepages: the store's read is `O_DIRECT`
into a registered frame, `O_DIRECT` needs a page-aligned buffer, and a heap `Vec` is not
aligned — so with no slab every store read silently takes the BUFFERED descriptor at 26.9-34.9
ms per 16 MiB against 17.3 direct, through the page cache the store exists to bypass. The
daemon refuses that combination outright (`assert_store_has_a_slab`), so an unscoped
`diskTier: store` here would make the chart's OWN defaults (`efa.enabled: false`) fail to
start. Defaulting it on exactly where a slab is derived is what makes the flip safe, and it is
the same mistake `scatter.enabled` shipped once as a bare `true`.

Why flip at all, when ADR-0033 gate 33.5 still FAILS: 33.5 is an ABSOLUTE bar (≥25 GiB/s) and
is unmet either way. The default is a CHOICE BETWEEN THE TWO tiers, and on that question one
node measured store at 23.750 GiB/s against foyer's 14.5 (1.633x) while foyer served 0.005 % of
its bytes from the drives at all — `bench/ladder/results/mountpoint-vs-pacer.md`. A reader
deploying the chart got the slower tier by default.

Resolution order, and an operator always wins:
  1. config.diskTier set explicitly -> that value, including `foyer` as the regression path.
  2. a slab is derived (pacer.cacheSlabBytes non-empty and non-zero) -> `store`.
  3. otherwise -> `foyer`, which is what is measured on a node with no slab.
*/}}
{{- define "pacer.diskTier" -}}
{{- $explicit := .Values.config.diskTier -}}
{{- if and (not (kindIs "invalid" $explicit)) (ne ($explicit | toString | trim) "") -}}
{{- $explicit | toString | trim -}}
{{- else -}}
{{/*
An explicit `cluster.cacheSlabBytes=0` is the documented "run without the slab" hatch and
resolves to the string "0" here, which is NOT no-slab to a bare `if` — Go templates call the
empty string falsey but "0" truthy. Compare as an integer so the hatch reaches `foyer`.
*/}}
{{- $slab := include "pacer.cacheSlabBytes" . -}}
{{- if and (ne (trim $slab) "") (gt ($slab | default "0" | int64) 0) -}}
{{- print "store" -}}
{{- else -}}
{{- print "foyer" -}}
{{- end -}}
...
{{/*
The pod resource name for `efa.hugepageSizeMib` — `hugepages-2Mi` or `hugepages-1Gi`.

ONE helper for it, used by the DaemonSet's request, because the resource name and the
daemon's `rdma-arena-page-mib` are two spellings of one decision and the failure mode when
they disagree is silent: `ArenaPages::from_mib` maps an unrecognised size to BASE pages, so
the pod would hold a 1 GiB reservation the arenas never map from and every registration would
run on 4 KiB pages — measurably slower (ADR-0028: ~19 GB/s on hugepages against 12-14 on base)
with nothing in the manifest saying so.

Which is also why an unknown value is REFUSED here rather than defaulted: 2 and 1024 are the
only sizes the transport encodes (`MAP_HUGE_SHIFT` 21 and 30), and a chart that quietly turned
`hugepageSizeMib: 4` into base pages would be the exact silent degradation this guard exists to
stop. x86-64 and arm64 both offer only these two through MAP_HUGETLB.
*/}}
{{- define "pacer.hugepageResource" -}}
{{- $mib := .Values.efa.hugepageSizeMib | default 2 | int -}}
{{- if eq $mib 2 -}}
hugepages-2Mi
{{- else if eq $mib 1024 -}}
hugepages-1Gi
{{- else -}}
{{- fail (printf "efa.hugepageSizeMib=%d is not a hugepage size this transport can map: only 2 (hugepages-2Mi) and 1024 (hugepages-1Gi) are encodable (MAP_HUGE_SHIFT 21 and 30, ArenaPages::from_mib). Anything else becomes BASE pages inside the daemon while the pod still reserves hugepages, so the reservation is held and never used — a slower daemon with nothing in its manifest to explain it. Pick 2 or 1024, and make sure the node reserves THAT size at boot (hugepages-2048kB or hugepages-1048576kB in the nodeclass userData)." $mib) -}}
{{- end -}}
{{- end -}}
