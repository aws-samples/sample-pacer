...
24Gi limit is exactly what cost a 256 GiB run
(bench/ladder/results/c4-dcp-256gib.md), but a chart cannot tell a deliberately tiny tier
on a large limit from an under-sized one: bench/ladder/values-ladder.yaml runs a 128 MiB
tier on purpose, and deploy/helm/pacer/values-dev-gpu.yaml sits at exactly 1x when layered
onto the base alone. So this fails only where the arithmetic is unambiguous, and the ~2x
rule of thumb in the docs closes the rest.
