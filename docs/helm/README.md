...
renders with grep assertions. See `ci/README.md`.

## Layered values files

The base chart is a complete, working install. Your own values layer onto it with `-f`:

```
deploy/helm/pacer/values.yaml                 the base — every key, with defaults
  + values-example.yaml                       a complete worked example: copy and edit
  + <your-values>.yaml                        anything you override
```

One example ships, not a family of them, because an overlay is only ever useful against
the cluster it was written for: a values file that names a nodepool you do not have
selects zero nodes, and a DaemonSet with `desired=0` reports a **successful** rollout on
zero pods. So `values-example.yaml` is the one file to copy, and the optional EFA block
lives commented inside it rather than as a second file to layer.
