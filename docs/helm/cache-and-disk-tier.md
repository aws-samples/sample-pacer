...
`/etc/pacer/config.yaml` ([ADR-0013](../../planning/adr/0013-yaml-config-file-layered-over-env.md)).
...
- **`backendType`** (default `express`) — `"express"` (S3 Express One Zone directory
  bucket, same-AZ — the [ADR-0002](../../planning/adr/0002-s3-express-one-zone-same-az-backend.md)
  ...
  [ADR-0023](../../planning/adr/0023-pluggable-s3-backend-type-express-or-standard.md) /
  the D2 benchmark).
...
- **`maxObjectSize`** — optional whole-object admission cap
  ([ADR-0015](../../planning/adr/0015-chunk-granular-caching.md)). Unset (the default) →
...
[`c4-foyer-readpath.md`](../../bench/ladder/results/c4-foyer-readpath.md) measured
...
([`c4-fanout-depth.md`](../../bench/ladder/results/c4-fanout-depth.md)) and why the
...
- It is derived rather than left to foyer's own default because that default is 16 MiB,
  ...
  [`c4-foyer-readpath.md`](../../bench/ladder/results/c4-foyer-readpath.md) recorded
...
- The depth term is fixed at **4**, because that is the reader look-ahead the C4 ladder
  ...
  ([`c4-fanout-depth.md`](../../bench/ladder/results/c4-fanout-depth.md): depth 4 is
...
([ADR-0033](../../planning/adr/0033-chunk-store-owns-the-disk-tier.md)): `"foyer"` (an
entry per chunk) or `"store"` (one `pread` into a registered frame). Empty → `foyer`,
i.e. exactly what the daemon did before ADR-0033, because the two tiers have different
integrity properties and a rollout must not move one silently.

The measured case for `"store"`: the device reads a 16 MiB chunk in **1.335 ms** and the
array gives **43.565 GiB/s**
([`nvme-device-truth.md`](../../bench/ladder/results/nvme-device-truth.md)), while foyer
charges **40-75 ms** per 16 MiB entry and caps the tier near 8 GiB/s. Object headers stay
in foyer either way, and `promotion` above stops applying.
...
- [ADR-0013 — YAML config file layered over env](../../planning/adr/0013-yaml-config-file-layered-over-env.md)
- [ADR-0015 — Chunk-granular caching](../../planning/adr/0015-chunk-granular-caching.md)
- [ADR-0023 — Pluggable S3 backend type (Express or Standard)](../../planning/adr/0023-pluggable-s3-backend-type-express-or-standard.md)
- [ADR-0033 — Chunk store owns the disk tier](../../planning/adr/0033-chunk-store-owns-the-disk-tier.md)
