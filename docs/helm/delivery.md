...
## What it is, and why it ships on
...
On by default because the behaviour is gated strictly on the request header: a client that sends none is served exactly as it was before, so the default cannot change an existing workload. It is still a privilege rather than a tuning knob — the daemon maps a segment a *client* named and pins its pages, and on an EFA node hands a peer an rkey into them — so what the default carries is two costs, neither of them behavioural, and both are what `delivery.enabled: false` gives back:

1. `sharedShm` — the client pod and the daemon must see the SAME tmpfs, which means a hostPath, NOT an emptyDir: an emptyDir (even `medium: Memory`) is created per POD, so sharing one across pods is impossible and every target would look absent (a 4xx, not a silent wrong read). This mounts `shmHostPath` at /dev/shm in the daemon; the client pod must mount the same host path. Consequence to weigh, and it is now a DEFAULT rather than something an operator asked for: any pod that mounts that tmpfs sees every segment on the node, and the daemon is one of those pods. Segment-name validation and the pinned-byte quotas are what bound the exposure, not the mount (ADR-0026 § New trust surface). The volume is rendered only when `enabled` AND `sharedShm` are both true, so `sharedShm: false` removes it and leaves `nic:` targets working — that is the setting for a cluster where the daemon should not be able to open other pods' segments at all.
...
The rendered ConfigMap's own comment on `delivery.enabled` restates the same argument at the point the key is emitted: a cooperating client names a shm segment it owns and the daemon delivers into it, answering with a header-only 200; clients that send no `x-pacer-target` header are unaffected, which is why this ships ON. It does mean the daemon maps and PINS memory a client named, so the cost of the default is not behavioural: `delivery.pinnedReservation` and the derived working set are ADDED to the container memory limit, which is what `delivery.enabled: false` gets back.
...
* the node has boot-reserved hugepages (the cache nodepool's EC2NodeClass userData reserves 2Mi pages; ADR-0028 needs them too);
...
WHY 4Gi is the number. It is ~2x the largest tensor measured, so one quota value outlasts several rungs instead of being re-tuned per model (a 405B-class hidden size takes the embedding past 2 GiB). And it costs the node nothing by itself: the bound on pinned client memory is `pinnedBytesMax`, which this is subordinate to — raising the per-request ceiling changes how much of the node-wide budget ONE GET may take, never the budget.

⚠ AND SINCE 2026-09-16 THE TWO ARE EQUAL AT 4Gi, which is the configuration this section used to argue against. When delivery went on by default, `pinnedBytesMax` was pinned to its floor rather than left at the daemon's 8Gi, because it lands in the container memory limit and the alternative was shipping a 13Gi limit to every install (see the sizing note in values.yaml). Equal ceilings mean **no isolation between concurrent delivering GETs**: one maximal window holds the entire node-wide budget and every concurrent one is declined into a body read — correct bytes, the slow path, and the tell is the metric below rather than an error.

That is a floor, not a recommendation. The rule for sizing it up, and both values move as a pair: **N concurrent maximal windows needs `pinnedBytesMax` = N × `maxTargetBytes`, with `pinnedReservation` matching it** — so a TP=8 loader delivering to eight ranks on one node wants 32Gi and 32Gi, and `pacer.memoryLimit` adds the reservation to the container limit on top of the base. Every bench overlay on record raises both; the shipped pair is sized to deliver a real checkpoint at all, not to run a rank-parallel loader at speed.
