...
([ADR-0008](../../planning/adr/0008-grpc-first-build-order.md),
[ADR-0012](../../planning/adr/0012-owner-read-through-cluster-fill.md)). Disabled = a
fleet of independent Phase 1 nodes.

`cluster.channelCapacity` (default **8**) is the number of chunks buffered per peer blob
stream ([ADR-0013](../../planning/adr/0013-yaml-config-file-layered-over-env.md)). Small
on purpose: the stream itself is the backpressure.

`cluster.replicationR` is the co-home count R
([ADR-0016](../../planning/adr/0016-multi-copy-replication.md)). Empty → the daemon default
...
Phase 3's EFA-RDMA path is off in the base chart and on in
[`values-efa.yaml`](../../deploy/helm/pacer/values-efa.yaml).

The A4 benchmark cleared the ADR-0008 flip gate
([`planning/14-benchmarks-a4-efa.md`](../../planning/14-benchmarks-a4-efa.md), 2026-07-21):
the holder-driven WRITE data plane beats gRPC on throughput AND holder CPU at high
fan-out, so EFA is now the intended default *wherever EFA hardware exists* — layer
`values-efa.yaml` (which sets `enabled: true`) onto an EFA nodepool. The base default
...
([ADR-0018](../../planning/adr/0018-holder-driven-rdma-write-data-plane.md)), and every
RDMA error falls back to gRPC per-peer
([ADR-0003](../../planning/adr/0003-efa-rdma-cross-node-reads-only-grpc-fallback.md)) —
...
([`planning/11-efa-a2-infra-runbook.md`](../../planning/11-efa-a2-infra-runbook.md)
finding #7):
...
* **`efa.hugepages` is NOT a throughput knob.**
  [`planning/18-p5-bandwidth-investigation.md`](../../planning/18-p5-bandwidth-investigation.md)
  ...
  ([ADR-0024](../../planning/adr/0024-registered-arena-rdma-buffers.md) point 3). Keep it
...
([`bench/ladder/results/c5-slab-zerocopy-source.md`](../../bench/ladder/results/c5-slab-zerocopy-source.md)).
So on a pool whose nodes DO pre-reserve 2 MiB pages, setting `efa.hugepages` is the largest
delivery win available — see
[`values-dev-gpu.yaml`](../../deploy/helm/pacer/values-dev-gpu.yaml), which sets it for
exactly that reason. The ceiling is the boot reservation, not the value: the `s3-ai-cache`
userData reserves 16 GiB, which at the 16 MiB chunk covers a tier of only ~4 GiB (arenas take
8), so flipping it for the cache pool needs a bigger reservation first; `gpu-az*` reserves
`32768 × 2Mi` = 64 GiB, so a tier above ~52 GiB cannot be slab-backed there at all.
...
[`spike/cdi/README.md`](../../spike/cdi/README.md) for why the CDI-by-annotation shortcut
is not available.
...
([`planning/16-rdma-saturation.md`](../../planning/16-rdma-saturation.md) §4.5 OOM
...
[`planning/19-saturation-roadmap.md`](../../planning/19-saturation-roadmap.md) § C1b).
...
([`bench/ladder/results/adr28-churn-gate.md`](../../bench/ladder/results/adr28-churn-gate.md)).
...
against the 64Gi the gpu-az* EC2NodeClasses currently reserve. `pacer.validateHugepages`
...
[ADR-0025](../../planning/adr/0025-rail-numa-placement-and-pinned-reapers.md)). Empty → on.
...
* [cache-and-disk-tier.md](cache-and-disk-tier.md) — `chunkSize`, which sizes the holder
  arena and the slab.
* ADRs [0018](../../planning/adr/0018-holder-driven-rdma-write-data-plane.md),
  [0021](../../planning/adr/0021-efadv-ibverbs-not-libfabric.md),
  [0022](../../planning/adr/0022-gpudirect-hbm-target-and-multi-rail-efa.md),
  [0024](../../planning/adr/0024-registered-arena-rdma-buffers.md),
  [0025](../../planning/adr/0025-rail-numa-placement-and-pinned-reapers.md),
  [0028](../../planning/adr/0028-cache-ram-tier-is-the-registered-arena.md),
  [0030](../../planning/adr/0030-delivery-registration-belongs-to-the-memory-owner.md).
