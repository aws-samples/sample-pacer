...
([`bench/ladder/results/c4-dcp-256gib.md`](../../bench/ladder/results/c4-dcp-256gib.md)).
...
([`bench/ladder/results/c4-fanout-depth.md`](../../bench/ladder/results/c4-fanout-depth.md)):
...
[`planning/17-phase4-plan.md`](../../planning/17-phase4-plan.md)'s requester OOM has
...
kept. [`planning/06-roadmap.md`](../../planning/06-roadmap.md)'s own conclusion is that
...
quantity** rather than a byte count — which is what keeps this arithmetic off every
install that never asked for any of it.
...
Every overlay in this repo satisfies it — checked over 34 renders (`values.yaml`, all
`deploy/helm/pacer/values-*.yaml`, all `bench/ladder/values-*.yaml` layered on
`values-ladder.yaml`, and the three `pacer-dev` layerings), most with 3-10× margin. The
one render that does not is **`values-dev-gpu.yaml` on its own**: a 4 GiB tier + 8 GiB
arenas + 8 GiB slab is 20 GiB against a 20 GiB limit, i.e. base == tier — the exact case
`pacer.validateMemoryBudget` above cannot fail at render time, and the exact file it names
as why. `pacer-dev` always layers it onto `values-gpu-ohio.yaml`, whose 8 GiB base leaves
2 GiB of slack, so no deployed configuration is refused. Render it alone to read the
arithmetic; do not install it alone.
...
[ADR-0035](../../planning/adr/0035-daemon-resources-and-qos-class.md) and summarised in
...
* [`planning/adr/0024-registered-arena-rdma-buffers.md`](../../planning/adr/0024-registered-arena-rdma-buffers.md),
  [`planning/adr/0028-cache-ram-tier-is-the-registered-arena.md`](../../planning/adr/0028-cache-ram-tier-is-the-registered-arena.md),
  [`planning/adr/0032-write-scatter-populates-the-cache.md`](../../planning/adr/0032-write-scatter-populates-the-cache.md),
  [`planning/adr/0033-chunk-store-owns-the-disk-tier.md`](../../planning/adr/0033-chunk-store-owns-the-disk-tier.md).
