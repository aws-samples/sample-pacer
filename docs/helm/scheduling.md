...
`scripts/dev/pacer-session` derives the token and claims
...
`key: null` deletion only works against chart DEFAULTS, not between `-f` overlay files, so
when you pass two `-f` files and the later one nulls a pool label the earlier one set, that
null would otherwise reach the manifest and render an API-invalid null value. Second, it
adds `pacer.io/session: <id>` when
...
Dev iteration mode (`scripts/dev/`): replaces the distroless image
...
(measured 2026-08-22; the dev loop had never produced a running daemon). The
